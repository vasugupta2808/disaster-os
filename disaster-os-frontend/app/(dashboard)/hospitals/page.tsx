...
export default function HospitalsPage() {
  const hookResult = useNearbyPlaces("hospital");
  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col gap-4">
      <div className="flex shrink-0 items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Find nearby medical facilities</p>
        </div>
      </div>
      <div className="flex-1 min-h-0">
        <NearbyPlacesMap placeType="hospital" hookResult={hookResult} />
      </div>
    </div>
  );
}
